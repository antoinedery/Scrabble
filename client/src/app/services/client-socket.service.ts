...
export class ClientSocketService {
    ...
    playerScoreListObservable: Subject<PlayerScore[]> = new Subject<PlayerScore[]>();
    playerNameListObservable: Subject<PlayerName[]> = new Subject<PlayerName[]>();
    connectedServerObservable: Subject<boolean> = new Subject();
    updateDictionariesObservable: Subject<DictionaryDetails[]> = new Subject<DictionaryDetails[]>();
    dictionaryDownloadObservable: Subject<IDictionary> = new Subject<IDictionary>();
    roomEndedByDictionaryRemoval: Subject<unknown> = new Subject();

    multiToSoloObservable: Subject<MultiPlayerGameInfos> = new Subject<MultiPlayerGameInfos>();
    ...
    initializeHomeListeners(): void {
        ...
        this.socket.on('updateIsMyTurnToPlay', (socketID: string) => {
            this.selectGameModeService.onlinePlayerTurnObservable.next(socketID === this.socket.id);
        });
        this.socket.on('hereIsTheReserveSize', (letterReserveTotalOfRoom: number) => {
        ...
        this.socket.on('hereIsAPlayerScore', (playerScore: number, playerSocketID: string) => {
            this.updatePlayerScore(playerScore, playerSocketID);
        });
        this.socket.on('hereIsLetterRackSize', (playerRackSize: number, playerSocketID: string) => {
        ...
        this.socket.on('hereIsTheReserve', (reserve: string) => {
            ...
            for (const letter of reserveMap.keys()) {
                ...
                this.displayMessageService.addMessageList('system', letter + `: ${quantity}` + '\n');
            }
        });
        this.socket.on('hereAreTheObjectives', (privateObjective: string, publicObjectives: string) => {
            const privateObjectiveMap: Map<number, Objective> = JSON.parse(privateObjective);
            const publicObjectivesMap: Map<number, Objective> = JSON.parse(publicObjectives);
            this.playerSettingsService.privatePlayerObjectiveObservable.next(privateObjectiveMap);
            this.playerSettingsService.publicObjectivesObservable.next(publicObjectivesMap);
        });
        ...
        this.socket.on('hereAreTheWaitingGames', (waitingGames: WaitingGame[], isALog2990Game: boolean) => {
        ...
        this.socket.on('cannotJoinGame', () => {
            this.matDialog.closeAll();
            const errorMsg = "Vous ne pouvez pas rejoindre cette partie : soit l'hôte l'a annulée, soit un autre joueur l'a rejointe avant vous.";
            alert(errorMsg);
        });
        this.socket.on('sendBothPlayerNames', (bothPlayerNames: string[]) => {
            ...
            const isCurrentPlayerTheGameCreator = this.player.name === this.gameRoom.gameCreatorName;
    ...
    initializeDatabaseListeners(): void {
        ...
        this.socket.on('updateClientDictionary', (dictionaryList: DictionaryDetails[]) => {
    ...
    placeLetter(placement: Placement, playerLetterRack: Tile[], message: string): void {
    ...
    getWaitingGames(): void {
        this.socket.emit('sendWaitingGames', this.selectGameModeService.isLOG2990ModeChosen);
    }
    sendChatMessage(message: string): void {
        this.socket.emit('newMessageToShare', message, this.player.name);
    }
    getLetterQuantityMulti(letter: string, reserve: Map<string, Reserve>): number | undefined {
    ...
    updatePlayerScore(playerScore: number, playerSocketID: string): void {
        if (this.socket.id === playerSocketID) this.playerScoreObservable.next(playerScore);
        else this.opponentScoreObservable.next(playerScore);
    }
    updatePlayerRackSize(playerLetterRackSize: number, playerSocketID: string): void {
        if (this.socket.id === playerSocketID) this.playerRackSizeObservable.next(playerLetterRackSize);
    ...
    editDictionary(previousDetails: DictionaryDetails, newDetails: DictionaryDetails) {
    ...
    getDictionaryForClient() {
        this.socket.emit('getDictionaryForClient', this.dictionaryService.dictionaryName);
    }
}
